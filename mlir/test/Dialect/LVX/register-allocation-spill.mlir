...
// CHECK-NEXT: %5 = lvx.ld %2, 0 : (!lvx.reg<r12>) -> !lvx.reg<r29>
// CHECK-NEXT: %6 = lvx.addd %5, %4 : (<r29>, <r0>) -> <r29>
// CHECK-NEXT: lvx.sd %6, %2, 8 : (<r29>, <r12>)
// CHECK-NEXT: %7 = lvx.ld %2, 0 : (!lvx.reg<r12>) -> !lvx.reg<r29>
...
// CHECK: lvx.sd %{{[0-9]+}}, %{{[0-9]+}}, 8 : (<r29>, <r12>)
// CHECK: lvx.sd %{{[0-9]+}}, %{{[0-9]+}}, 16 : (<r29>, <r12>)
// CHECK: ^bb1:
// CHECK-NEXT: %{{[0-9]+}} = lvx.ld %{{[0-9]+}}, 8 : (!lvx.reg<r12>) -> !lvx.reg<r29>
// CHECK-NEXT: %{{[0-9]+}} = lvx.mv %{{.*}} : (!lvx.reg<r29>) -> !lvx.reg<r0>
// CHECK: lvx_func.return
// CHECK: ^bb2:
// CHECK-NEXT: %{{[0-9]+}} = lvx.ld %{{[0-9]+}}, 16 : (!lvx.reg<r12>) -> !lvx.reg<r29>
// CHECK-NEXT: %{{[0-9]+}} = lvx.mv %{{.*}} : (!lvx.reg<r29>) -> !lvx.reg<r0>
// CHECK: lvx_func.return
